...
$sqlcon="SELECT*FROM producto inner join marca on producto.idMarca=marca.idMarca inner join proveedor on producto.idProveedor=proveedor.idProveedor inner join sucursal on sucursal.idSucursal=producto.idSucursal WHERE nombreProd like '%$nombreCliente%' OR codProd like '%$nombreCliente%' OR NombreMarca like '%$nombreCliente%' OR NombreProveedor like '%$nombreCliente%'  OR desProd like '%$nombreCliente%'";	
}else{
...
echo '<td><div id="opciones-'.$clientes['idProd'].'"><input onclick="agregarBajas('.$clientes['idProd'].');" class="btn btn-warning btn-sm" type="button" value="Agregar Bajas" title="seleccionar producto">';
echo '</div></td></tr>';    
}
echo "</table><br><br>";
}else{
