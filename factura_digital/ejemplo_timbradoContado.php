...
$conVenta=ejecutarSQL::consultar("SELECT*FROM venta_contado where NumFolio='".$_SESSION['folioVentaRealizada']."'");
...
if(consultasSQL::InsertSQL("mis_facturas","folio_factura,fecha_emision,idVenta,tipoVenta,idCliente,idVendedor,total_factura","'$folio','$fecha',$idVenta,'$tipoVenta',$idCliente,$idVendedor,$totalFactura")){
	try {
		...
		if($venta['tipoPago']==1 || $venta['tipoPago']==4){
        $formaPago='01';
		}else{
			$formaPago='28';
		}
		$d['FormaPago'] 		= $formaPago;
		$d['CondicionesDePago'] = 'Pago en una sola exhibición';
		...
		$descuento=$venta['descuento']/100*$venta['totalPagar'];
		echo $descuento."<br>";
		$descuentoUnitario=number_format($descuento/$numProd,2,".","");
		echo $descuentoUnitario."<br>";
		...
		if($prodres['siniva']==0){
		$ivau=$prodres['precioVenta']*0.16;
		$pu=$prodres['precioVenta']-$ivau;
		$d['Conceptos'][$x]['ValorUnitario'] =$pu;
		$importe=number_format($pu*$prods[1],2,'.','');
		$totalIva=$ivau*$prods[1];
		}else{
		$d['Conceptos'][$x]['ValorUnitario']=$prodres['precioVenta'];
		$importe=number_format($prodres['precioVenta']*$prods[1],2,'.','');    
		}
        ...
        $base=$importe;
		$d['Conceptos'][$x]['Descuento'] = $descuentoUnitario;
		}
		if($prodres['siniva']==0){		
	...
	//	}else{
		...
		$d['SubTotal'] 			= $subtotal;
		$d['Total'] 			=$subtotal-$descuento+$ivaFinal;
		/*
